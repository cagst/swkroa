...
/* package */final class MemberMapper implements RowMapper<Member> {
  ...
  public Member mapRow(final ResultSet rs, final int rowNum) throws SQLException {
    ...
    member.setJoinDate(LocalDateConverter.convert(rs.getTimestamp(JOIN_DT)));
    ...
    member.setCloseDate(LocalDateTimeConverter.convert(rs.getTimestamp(CLOSE_DT_TM)));
  ...
  private static void mapCommonProperties(final MapSqlParameterSource params,
    ...
    params.addValue(COMPANY_NAME_KEY, StringUtils.stripToNull(SwkroaStringUtils.normalizeToKey(member.getCompanyName())));
    ...
    params.addValue(JOIN_DT, LocalDateConverter.convert(member.getJoinDate()));
